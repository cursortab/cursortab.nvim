...
func TestBuildCompletionRequest_UsesSharedSnapshotBridge(t *testing.T) {
	...
	buf.originalLines = []string{"original"}
	buf.diffHistories = []*types.DiffEntry{
		{Original: "old current", Updated: "new current", Source: types.DiffSourceManual, TimestampNs: time.Now().UnixNano(), StartLine: 1},
	}
	prov := newMockProvider()
	clock := newMockClock()
	clock.now = time.Now()
	eng := createTestEngine(buf, prov, clock)
	eng.fileStateStore["recent.go"] = &FileState{
		FirstLines: []string{"recent"},
		DiffHistories: []*types.DiffEntry{
			{Original: "old recent", Updated: "new recent", Source: types.DiffSourceManual, TimestampNs: clock.now.UnixNano(), StartLine: 2},
		},
	...
	assert.Equal(t, input.Current.Cursor.Row, req.CursorRow, "request row")
